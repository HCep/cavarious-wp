...
function shortcode_slide(){
  ...
  if($query->have_posts()) :
	  ...
	  while($query->have_posts()) :
		$slide_image_mobile = get_field('slide_image_mobile');
		$slide_image = get_field('slide_image');
		  $query->the_post() ;
		  $result .= '<li class="splide__slide">';
		  if(!empty($slide_image_mobile)){
          $result .=  '<img class="mobile-hidden" src="'. get_field('slide_image')['url']  .'" alt="'. get_field('slide_image')['alt']  .'" />';
          $result .=  '<img class="desktop-hidden" src="'. get_field('slide_image_mobile')['url']  .'" alt="'. get_field('slide_image_mobile')['alt']  .'" />';}
		  else{
			$result .=  '<img src="'. get_field('slide_image')['url']  .'" alt="'. get_field('slide_image')['alt']  .'" />';
		  }
		  if (get_locale() == 'en_GB') {
			$result .= '<div class="slide__content"><div><h1 class="slide__heading text-white">'. get_field('slide_content')  .'</h1></div>';
		}else{
		$result .= '<div class="slide__content"><div><h1 class="slide__heading text-white">'. get_field('slide_content_pl')  .'</h1></div>';}
          $result .=  '</div></li>';
	  endwhile;
